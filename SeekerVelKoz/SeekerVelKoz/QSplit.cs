...
namespace SeekerVelKoz
{
    public static class QSplit
    {
        ...
        private static List<Vector2> Perpendiculars { get; }
                            ...
                            collisionObjects.Where(
                                o =>
                                    Prediction.Position.Collision.LinearMissileCollision(o, startPos, endPos,
                                        MissileSpeed, SpellWidth, CastDelay,
                                        (int) (o.BoundingRadius - o.BoundingRadius/2)))
                                .OrderBy(o => o.Distance(Player.Instance, true))
                                .FirstOrDefault();

                        if (colliding != null && colliding.IsValidTarget() &&
                            colliding.Type == GameObjectType.AIHeroClient)
                        {
                            if (VelKoz.Q.Cast())
                                Handle = null;
                        }
